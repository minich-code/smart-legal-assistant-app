...
async def lifespan(app: FastAPI):
    """
    Manages the application's lifespan. Initializes the RAG pipeline on startup.
    """
    logger.info("Application startup...")
    app.state.rag_pipeline = LegalRAGPipeline(ConfigurationManager())
    logger.info("LegalRAGPipeline initialized successfully.")
...
# --- API Endpoints ---
@app.get("/", tags=["Health"])
@app.head("/", tags=["Health"])  # Added to support Render's health check
async def read_root():
...
async def handle_query(request: Request, query_request: QueryRequest):
    """
    Receives a user query, processes it through the RAG pipeline, and returns the answer with citations.
    """
    try:
        logger.info(f"Received query for namespace '{query_request.namespace}': '{query_request.query[:100]}...'")
        pipeline: LegalRAGPipeline = request.app.state.rag_pipeline
...
# --- Main Entry Point ---
# if __name__ == "__main__":
#     # Use environment variable PORT for Render, fallback to 8000 for local development
